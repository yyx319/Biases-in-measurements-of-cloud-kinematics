...
thin = 0 # 0: including opacity effect; 1: neglecting opacity effect
thin2 = 0 # 1: neglecting Radiative transfer
    ...
    for time in time_a:
        ...
        for prj_ori, theta, phi in zip(prj_ori_a, theta_a, phi_a):
            ...
            for s in sp_index_a:
                ...
                file3 = "/avatar/yuxuan/tracer_ls_rel/molecule/%s/%s%s_lum_std.txt"%(sp2[s], sp1[s], '_thin'*thin)										#list of corresponding HCN luminosities per H molecule (log scale)
                ...
                cube.write_fits("/avatar/yuxuan/tracer_ls_rel/PPV/beta%s_%s_%s/cube_%s_%s_%s_L%s%s.fits"%(beta,time,prj_ori,beta, time, prj_ori, sp1[s],'_thin'*thin), overwrite=True, length_unit="pc")
                
                from astropy.io import fits
                cube = fits.open("/avatar/yuxuan/tracer_ls_rel/PPV/beta%s_%s_%s/cube_%s_%s_%s_L%s%s.fits"%(beta,time,prj_ori,beta, time, prj_ori, sp1[s],'_thin'*thin))   
                ...
                np.savetxt("%s/beta%s_%s_%s/m0_%s%s.txt"%(directory,beta,time,prj_ori,sp1[s], '_thin'*thin), m0)
                
                m1 = np.sum(cube*v, axis=0)/m0
                np.savetxt("%s/beta%s_%s_%s/m1_%s%s.txt"%(directory,beta,time,prj_ori,sp1[s], '_thin'*thin), m1)
               
                m2 = np.sum((v-m1)**2*cube, axis=0)/m0
                m2 = np.sqrt(m2)  
                np.savetxt("%s/beta%s_%s_%s/m2_%s%s.txt"%(directory,beta,time,prj_ori,sp1[s], '_thin'*thin), m2)
                
                print(r'\beta=%s, t=%s t_{ff} orientation: %s'%(beta, time, prj_ori))
                lw = np.average(m2, weights=m0)
                second_m.append(lw)  
            np.savetxt("%s/linewidth_%s_%s_%s.txt"%(directory,beta,time, prj_ori), second_m)   
